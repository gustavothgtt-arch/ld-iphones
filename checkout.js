    ...
    const WHATSAPP_LD = "5521974777097";
    ...
    function textoRecebimentoWhatsApp(valor) {
        return valor === "entrega" ? "Entrega" : "Retirada";
    }

    function textoPagamentoWhatsApp(valor) {
        const opcoes = {
            pix: "PIX",
            dinheiro: "Dinheiro",
            cartao: "Cartão"
    ...
    function montarMensagemWhatsApp(pedido, referencia) {
        const linhas = [
            "Olá! Acabei de finalizar um pedido pelo site da LD iPhones.",
            "",
            `*Pedido: ${referencia}*`,
            "",
        ...
        if (pedido.email) {
            linhas.push(`*E-mail:* ${pedido.email}`);
        }

        linhas.push("", "*Produtos:*");

        pedido.itens.forEach((item) => {
            const detalhes = [item.armazenamento, item.cor].filter(Boolean).join(" • ");

            linhas.push(
                `• ${item.quantidade}x ${item.nome_produto}${detalhes ? ` - ${detalhes}` : ""}`,
                `  ${moedaCheckout(item.subtotal)}`
            );
        });

        linhas.push(
            "",
            `*Total:* ${moedaCheckout(pedido.valor_total)}`,
            `*Recebimento:* ${textoRecebimentoWhatsApp(pedido.tipo_recebimento)}`
        );
        ...
        linhas.push(`*Pagamento:* ${textoPagamentoWhatsApp(pedido.forma_pagamento)}`);

        if (pedido.observacoes) {
            linhas.push("", `*Observações:* ${pedido.observacoes}`);
        }

        linhas.push("", "Gostaria de continuar o atendimento por aqui.");

        return linhas.join("\n");
    }

    function abrirWhatsAppPedido(pedido, referencia) {
        const mensagem = montarMensagemWhatsApp(pedido, referencia);
        const url = `https://wa.me/${WHATSAPP_LD}?text=${encodeURIComponent(mensagem)}`;
    ...
    function configurarEnvioPedido() {
        formCheckout.addEventListener("submit", async (evento) => {
            ...
            try {
                ...
                abrirWhatsAppPedido(pedido, referenciaBanco);
            } catch (erro) {
                console.error(erro);
